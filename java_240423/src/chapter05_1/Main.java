...
//		p2.swim();//업케스팅은 슈퍼클래스의 멤버만 접근 가능(오류 발생)
		p2.walk();
		Penguin p3 = (Penguin) p2;//downcasting
		p3.swim();//다운캐스팅은 모든 멤버 접근 가능
		
		Bird d = new Duck();//업캐스팅
		d.eat();//업캐스팅은 슈퍼클래스위 멤버만 접근 가능
		Duck d2 = (Duck) d;//다운캐스팅
		d2.fly();//다운캐스팅은 모든 멤버 접근 가능

		//추상클래스를 참조변수 선업(슈퍼클래스)
		Animal d3 = new Duck();//업케스팅은 슈퍼클래스의 멤버만 접근 가능
		int x = d3.life;
